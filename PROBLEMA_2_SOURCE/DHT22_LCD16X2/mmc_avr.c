...
#define CS_LOW()		PORTD &= ~CS	/* Set MMC_CS = low */
#define	CS_HIGH()		PORTD |= CS	/* Set MMC_CS = high */
...
{	
	...
	DDRB |= MOSI;		// Set PB3, MOSI to output
	DDRB &= ~MISO;		// Set PB4, MISO to input
	
	DDRB |= SCK;		// Set PB5, SCK to output
	DDRD |= CS;		// Set PD4, CS to output
	DDRB |= SS;		// Set PD4, SS to output
